...
- **终止会话** — 提前结束专注并记录已专注时间，区别于重置（清空不记录）
- **会话持久化** — 计时状态自动保存，关闭程序后重新打开可恢复进度
- **左侧导航** — 首页 / 事件 / 笔记 / 待办 / 统计 / 设置，选中态窄线指示器，页面内切换无弹窗
- **模拟时钟** — 实时时钟显示，带时针、分针、秒针，下方同步显示数字时间
- **圆弧进度条** — 计时器显示圆弧进度条，实时可视化当前段落剩余时间
- **图标控制** — ▶/⏸ 开始暂停切换、⏭ 跳过、■ 终止、↺ 重置
- **鼠标悬停提示** — 悬停按钮显示功能说明和快捷键，白底风格统一
- **事件** — 专注结束后自动记录，按日期分组显示（今天 / 昨天 / MM月DD日），支持搜索过滤、删除
- **笔记** — 手动添加笔记，支持标题 + 内容，双击编辑，右键菜单删除，支持搜索过滤，拖放至文件夹时高亮目标
- **笔记文件夹** — 创建文件夹归类笔记，左侧面板切换筛选，右键重命名 / 删除
- **文件夹可调宽度** — 拖动分隔线调整文件夹面板宽度，长名称自动省略号截断，悬停显示全称
- **待办事项** — 带复选框，勾选后划线并记录完成时间，文字占满整行，双击编辑，自动排序（未完成优先），支持一键清除已完成
- **历史统计** — 总体 / 本周 / 本月汇总，连续打卡天数，本周每日柱状图（悬停显示数值），按 ISO 周分组查看专注记录
- **事项统计** — 按事项聚合专注时长和次数，按时长降序排列
- **趋势折线图** — 近 14 天专注趋势可视化，窗口拉伸自适应，悬停显示日期和时长
- **逐条管理** — 统计页展示每条专注记录，支持逐条删除，统计汇总同步更新
- **数据导出** — 一键导出专注统计为 CSV 文件，方便外部分析
- **设置面板** — 声音提醒、窗口置顶、关闭时最小化到托盘、休息后自动开始、长休息配置、每日目标、闲置检测，分组显示
- **闲置检测** — 超过设定时间无鼠标键盘操作自动暂停计时器，专注数据更真实
- **每日目标** — 设定每日番茄目标，首页小圆点实时显示完成进度（如"今日 3/8"）
- **系统托盘** — 关闭窗口最小化到系统托盘，后台计时不中断，托盘图标实时显示倒计时，托盘菜单支持开始/暂停、恢复或退出
- **Windows 通知** — 专注结束和休息结束时弹出系统原生通知，即使最小化到托盘也不会错过
- **单实例** — 程序运行时再次打开会提示已在运行
- **键盘快捷键** — 空格（开始/暂停）、R（重置）、N（笔记）、T（待办）、S（跳过）、Escape（终止）、1-5（切换页面）
- **删除确认** — 删除事件、笔记、待办、文件夹均弹出二次确认弹窗
...
├── widgets.py       # 圆角 UI 组件（按钮、输入框、侧边栏按钮、确认弹窗、Windows 通知）
├── fig/             # Logo 和截图
...
详见 [RELEASE_v2.3.md](RELEASE_v2.3.md)

- 计时器升级为圆弧进度条，实时显示剩余时间
- 新增每日目标进度指示器，小圆点展示今日完成情况
- 柱状图和趋势图支持鼠标悬停显示数值
- 键盘快捷键扩展：S（跳过）、Escape（终止）、T（待办）、1-5（切页）
- 事件列表按日期分组显示
- 所有删除操作增加二次确认弹窗
- 笔记拖放时目标文件夹高亮
- 设置页分区显示（通用 / 计时节奏 / 闲置检测）
- 托盘菜单新增"开始/暂停"
- 修复计时器文字裁切和确认弹窗比例问题

### V2.2

详见 [RELEASE_v2.2.md](RELEASE_v2.2.md)

- 待办页面全面改进：文字占满整行，双击弹窗编辑，点击圆圈勾选
- 修复待办添加占位文字被当作内容的 bug
- 修复计时器跳秒和事件不记录（auto_start_focus 竞态问题）
- 笔记时间格式统一加年份，解决跨年排序问题
- 托盘退出线程安全修复
- timer tick 异常保护（try/finally）
- 文件夹迁移补漏，.gitignore 更新

### V2.1

详见 [RELEASE_v2.1.md](RELEASE_v2.1.md)

- 新增终止会话按钮，提前结束并记录已专注时间
- 新增会话持久化，关闭程序后恢复计时进度
- 新增事项统计，按事项聚合专注时长和次数
- 新增近 14 天趋势折线图，窗口宽度自适应
- 笔记文件夹面板支持拖动分隔线调整宽度
- 文件夹长名称省略号截断，悬停显示全称
- 跨年事件修复，事件时间包含年份
- 所有对话框支持 Enter 键确认
